...
**Participants:** `Web UI`, `AuthController`.
**Diagram:** [![SD level 2](sd/level-2/svg/US-005-level-2.svg)](sd/level-2/puml/US-005-level-2.puml)

## Level 3 - SD (detailed)

**Participants:** `AuthController`, `AuthService`, `IAccountRepository`, `PasswordHasher<Account>`, `ITokenService`.
**Diagram:** [![SD level 3](sd/level-3/svg/US-005-level-3.svg)](sd/level-3/puml/US-005-level-3.puml)

| Step | Sender → receiver | Operation | Outcome |
| --- | --- | --- | --- |
| 1 | UI → Controller | `POST /auth/sign-in` | Delegates to `AuthService.SignIn` |
| 2 | Service → AccountRepository | `FindByEmail` | Looks up the account; rejects if missing or SSO-only |
